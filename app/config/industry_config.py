...
for thresholds in INDUSTRY_THRESHOLDS.values():
    thresholds['Directors Score'] = DIRECTOR_SCORE_PASS_THRESHOLD


def get_sector_risk(industry: str) -> int:
    """Return the canonical binary sector risk flag for an app industry bucket."""
    return int(get_industry_thresholds(industry).get('Sector Risk', 1))


def get_industry_thresholds(industry: str) -> dict:
    """Return thresholds for a known industry, falling back to Other."""
    return INDUSTRY_THRESHOLDS.get(industry, INDUSTRY_THRESHOLDS['Other'])
